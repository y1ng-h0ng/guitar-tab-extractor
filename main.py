...
def main(argv=None):
    ...
    parser.add_argument("--debug-dir", help="保存谱面框、清洗前后截图、各行和拼接长图")
    args = parser.parse_args(argv)
    out = args.output or os.path.splitext(args.video)[0] + "_吉他谱.pdf"
    start = time.time()
    try:
        summary = run_pipeline(args.video, out, sample_fps=args.sample_fps, dpi=args.dpi, bars_per_row=args.bars_per_row, region=args.region, polarity=args.polarity, debug_dir=args.debug_dir)
